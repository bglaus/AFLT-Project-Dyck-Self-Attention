...
def attention(query, key, value, mask=None, dropout=None, hard_attention=False):
...
class MultiHeadedAttention(nn.Module):
	...
	def forward(self, query, key, value, mask=None, hard_attention=False):
		"Implements Figure 2"
		if mask is not None:
		...
		x, self.attn = attention(query, key, value, mask=mask, 
								 dropout=self.dropout, hard_attention=hard_attention)
